...
def actualizar_citas():
    ...
    cursor = db.cursor()
    cursor.execute("SELECT Nombre_completo ,Edad,Name,dd FROM tblcitas s join tblregistro d on s.id =d.id ")  # select * from cita join registro on cita.id = registro.id_cita
...
#     results = cursor.fetchall()
    return render_template('/main/prueba_mostrar_user.html', resultado=resultado)
